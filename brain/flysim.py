...
flybrainbnb additions (defaults keep the stock behaviour):
  - run(count_all=True) returns '_counts', uint16 spikes per neuron;
  - Params.tau_syn (ms, default None = stock instantaneous kick). When set, a
    presynaptic spike adds its weight to a per-neuron synaptic variable g that
    decays with tau_syn and drives the membrane as dv/dt = (g - (v - v_rest)) / tau_m,
    the current-based synapse of Shiu et al. 2024 (tau 5 ms). The state then
    carries 'g' too.
"""
import numpy as np
...
BUILD = Path(__file__).parent / "data"  # flybrainbnb: connectome files live in data/
...
class Params:
    ...
    refractory = 2.2    # ms
    dt = 0.2            # ms
    tau_syn = None      # ms; None = stock instantaneous kick (see module doc)
...
class FlyBrain:
    ...
    def run(self, drive, steps, gains=None, record=None, seed=0, spike_log=False,
        ...
        if state is None:
            ...
            gsyn = (np.array(state["g"], dtype=np.float32, copy=True)
                 if state.get("g") is not None else np.zeros(n, dtype=np.float32))
        ...
        all_counts = np.zeros(n, dtype=np.uint16) if count_all else None
        syn = p.tau_syn is not None
        if syn:
            g_decay = np.float32(np.exp(-p.dt / p.tau_syn))
            g_in = np.float32(p.dt / p.tau_m)
        ...
        for _ in range(steps):
            ...
            if len(fired):
                ...
                if tot:
                    ...
                    inc = np.bincount(tgt, weights=val, minlength=n).astype(np.float32)
                    if syn:
                        gsyn += inc
                    else:
                        v += inc
        ...
        out["_state"] = {"v": v, "refr": refr, "rng": rng.bit_generator.state,
                         "g": gsyn if syn else None}
        if count_all:
            out["_counts"] = all_counts
        if spike_log:
            out["_spikes"] = log
        return out
